    ...
    def rec_build_tree(self, node, p, plist, visited):
        ...
        if p in self.branchingpoints:
            if p is self.tree_root.coords:
                ...
                new_node = Node(p, sorted(self.contact_dict[self.skel_list.index(p)]), True, False, self.image[p])
                plist.append(p)
        ...
        # p is end point: create new Node and stop recursion
            ...
            new_node = Node(p, sorted(self.contact_dict[self.skel_list.index(p)]), False,True, self.image[p])
            plist.append(p)
            new_node.fatherbranch = plist
            node.children.append(new_node)
            new_node.father =node
    ...
    def build_tree(self):
        ...
        try:
            ...
            if self.check_directions() is False:
                ...
                self.tree_root = Node(root, sorted(self.contact_dict[self.skel_list.index(root)]), True, False, self.image[root])
